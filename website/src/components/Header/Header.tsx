        ...
        <nav className="bg-white border-gray-200 dark:bg-gray-900">
            <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
                <Link href="/" className="flex items-center space-x-3 rtl:space-x-reverse">
                    <Image src={"/palette-svgrepo-com.svg"} width={48} height={48} alt="Flowbite Logo" />
                    <span className="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">{title}</span>
                </Link>
                
                {session.isLoggedIn && <li><p className="font-medium text-gray-900 rounded dark:text-white">Welcome {session.username}</p></li>}
                ...
                <ul className="font-medium flex flex-col p-4 md:p-0 mt-4 border border-gray-100 rounded-lg bg-gray-50 md:flex-row md:space-x-8 rtl:space-x-reverse md:mt-0 md:border-0 md:bg-white dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
                    ...
                    {session.isLoggedIn && <li><LogoutForm/></li>}
                </ul>
                </div>
            </div>
        </nav>
